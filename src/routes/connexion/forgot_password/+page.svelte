<script>
  ...
  $: email && console.log("Email valide :", checkEmail(email));
  ...
  ///// pour l'otp (pas eu le choix le component ne repond pas correctement)

  let otp = ["", "", "", "", "", ""];

  function checkOtp(otpArray) {
  ...
  const handleSubmitOtp = async (event) => {
    ...
    if (!checkOtp(otp)) {
      ...
      if (res.data.expired == false) {
        sessionStorage.setItem("otp", JSON.stringify(otp.join("")));
        otpSubmitted = true;
        isLoading = false;
      } else {
  ...
  const handleSubmitMdp = async (event) => {
    event.preventDefault();
    isLoading = true;
  ...
  function handleInput(e, i) {
    ...
    otp[i] = value;
    // Si une valeur est saisie, passe au champ suivant
    if (value && i < 5) {
...
<main>
  {#if emailSubmitted == false}
    <div
      class="min-h-screen bg-[url('/bg5.jpg')] bg-cover bg-center bg-no-repeat min-h-screen flex items-center justify-center flex items-center justify-center px-4"
    >
      <div class="max-w-md w-full bg-white rounded-2xl shadow-lg p-8">
        <div class="text-center mb-8">
          ...
          >
            <i
              class="ri-shield-check-line text-blue-500 text-xl w-6 h-6 flex items-center justify-center"
            ></i>
          </div> -->
        </div>
        <div class="text-center mb-8">
        ...
        <form class="space-y-6" on:submit|preventDefault={handleSubmit}>
          ...
          <button
            type="button"
            disabled={!checkEmail(email) || isLoading}
  ...
  {:else if emailSubmitted && otpSubmitted == false}
    <div class="bg-[url('/bg5.jpg')] bg-cover bg-center bg-no-repeat min-h-screen  flex items-center justify-center px-4">
      <div class="max-w-md w-full bg-white rounded-2xl shadow-lg p-8">
        <div class="text-center mb-8">
          <div class="text-3xl font-['Pacifico'] text-blue-500 mb-2">
            <img
        ...
        {/if}
        <form class="space-y-6" on:submit|preventDefault={handleSubmitOtp}>
          <div style="display: flex; gap: 10px;">
            {#each otp as val, i}
              <input
                ...
                class="w-1/6 px-4 py-3 border rounded-lg text-center"
              />
            {/each}
          </div>
          <div class="text-sm text-right mb-4">
            ...
            <button
              type="submit"
              disabled={!checkOtp(otp) || isLoading}
              class="w-full bg-blue-500 text-white py-3 px-4 rounded-lg font-medium hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
            >
              {#if isLoading}
                Vérification...
  ...
  {:else if emailSubmitted && otpSubmitted}
    <div class="bg-[url('/bg5.jpg')] bg-cover bg-center bg-no-repeat min-h-screen  flex items-center justify-center px-4">
      <div class="max-w-md w-full bg-white rounded-2xl shadow-lg p-8">
        <div class="text-center mb-8">
          <div class="text-3xl font-['Pacifico'] text-blue-500 mb-2">
        ...
        <div class="text-center mb-8">
          ...
          <p class="text-gray-600 text-sm">Entrez votre nouveau mot de passe</p>
        </div>
        <form class="space-y-6" on:submit|preventDefault={handleSubmitMdp}>
          <div>
            <label
              for="password"
              class="block text-lg font-medium text-gray-700 mb-2"
          ...
          <div>
            ...
            ><input
              ...
              required={true}
              bind:value={confirmMdp}
            />
          </div>
          <button
            type="submit"
            disabled={!checkMdp(mdp) || mdp !== confirmMdp || isLoading}
            class="w-full bg-blue-500 text-white py-3 px-4 rounded-lg font-medium hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
          >
            {#if isLoading}
              Chargement...
        ...
        <div class="text-center mt-6">
          <a
            class="text-blue-500 text-sm hover:text-blue-600 transition-colors cursor-pointer"
            href="/connexion">← Back to Login</a
          >
        </div>
      </div>
    </div>
