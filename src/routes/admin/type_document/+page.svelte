<script lang="ts">
  ...
  const path: string = '/admin/type_document';
...
>
  <Abercrome titre="Parametres" parent="Dashbord" current="type document" />
  <!-- Responsive Toggler -->
    ...
    >
      ...
      <div class="p-[20px]">
        <div class="scrollbar overflow-x-auto">
          ...
          >
            <!-- Header -->
            <HeaderTable item={["nombre", "libelle", "Type personne",]} />
            <!-- Body -->
              ...
              {:else}
                {#each paginatedData as item, i}
                  <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 text-[14px]  py-3 border border-gray-200">{item.nombre}</td
                    >
                    <td class="px-4 text-[14px]  py-3 border border-gray-200"
                      >{item.libelle}</td
                    >
                    <td class="px-4 text-[14px]  py-3 border border-gray-200"
                      >{item.typePersonne.libelle}</td
                    >
...
<Modale bind:open={openAdd} size="xl" title="Créer un type document">
  <Add bind:open={openAdd} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openEdit} size="xl" title="Modifier un type document">
  <Edit bind:open={openEdit} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openShow} size="xl" title="Détails d'un type document">
