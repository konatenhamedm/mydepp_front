<script lang="ts">
  ...
  const path: string = '/admin/devise';
  ...
  async function fetchData() {
    loading = true;
    try {
      const res = await apiFetch(true, `/devises/all?page=${currentPage}`);

      if (res) {
        main_data = res.data as Devise[];
...
>
  <Abercrome titre="devise" parent="Dashbord" current="devise" />
  <!-- Responsive Toggler -->
    ...
    >
      ...
      <div class="p-[20px]">
        <div class="scrollbar overflow-x-auto">
          ...
          >
            <!-- Header -->
            <HeaderTable item={['Code', 'symbole', 'Nbre decimal']} />
            <!-- Body -->
...
<Modale bind:open={openAdd} size="xl" title="Créer une devise">
  <Add bind:open={openAdd} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openEdit} size="xl" title="Modifier une devise">
  <Edit bind:open={openEdit} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openShow} size="xl" title="Détails d'une devise">
  <Show bind:open={openShow} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openDelete} size="xl" title="Supprimer devise">
