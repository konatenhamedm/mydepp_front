<script lang="ts">
  ...
  const path: string = '/admin/destinateur';
...
>
  <Abercrome titre="Parametres" parent="Dashbord" current="destinateur" />
  <!-- Responsive Toggler -->
...
<Modale bind:open={openAdd} size="xl" title="Créer un destinateur">
  <Add bind:open={openAdd} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openEdit} size="xl" title="Modifier un destinateur">
  <Edit bind:open={openEdit} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openShow} size="xl" title="Détails d'un destinateur">
