<script lang="ts">
  ...
  const path: string = '/admin/dossiers-acceptes';
  ...
  async function fetchData() {
    loading = true;
    try {
      const res = await apiFetch(true, "/professionnel/accepte");
      if (res) {
  ...
  $: filteredData = main_data.filter((item) => {
    return (
      ...
      item.personne?.number.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    >
      ...
      <div class="p-[20px]">
        <div class="scrollbar overflow-x-auto">
          ...
          >
            <!-- Header -->
            <HeaderTable item={['Nom', 'Prénoms', 'Téléphone', 'Email', 'Type']} />

            <!-- Body -->
            <tbody class="text-gray-700">
              {#if loading && paginatedData.length === 0}
                <tr>
                  <td colspan="6" class="py-8 text-[14px] text-center text-gray-500">
