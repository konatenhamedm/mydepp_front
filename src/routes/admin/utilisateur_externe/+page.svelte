<script lang="ts">
  ...
  const path: string = '/admin/administrateurs';
  ...
  async function fetchData() {
    loading = true;
    try {
      const res = await apiFetch(true, '/user/get/user/externe');
  ...
  onMount(async () => {
    user = getAuthCookie();
    await fetchData();
    permission = 'CRUD'; // À adapter selon vos règles d'authentification
  ...
  $: filteredData = main_data.filter((item) => {
    return item.username.toLowerCase().includes(searchQuery.toLowerCase()) ||
           ...
           item.typeUser?.toLowerCase().includes(searchQuery.toLowerCase());
  });
...
<div class="ssm:mt-[30px] mx-[30px] mt-[15px] mb-[30px] min-h-[calc(100vh-195px)]">
  <Abercrome titre="Utilisateurs " parent="Paramètres" current="Gestion des utilisateurs externes" />
  
  <div class="col-span-12">
    <div class="dark:bg-box-dark text-body dark:text-subtitle-dark rounded-10 relative m-0 bg-white p-0 text-[15px] shadow">
      <!-- En-tête -->
      <div class="text-dark dark:text-title-dark border-regular dark:border-box-dark-up flex flex-wrap items-center justify-between border-b px-[25px] text-[17px] font-medium max-sm:h-auto max-sm:flex-col">
        <h1 class="text-dark dark:text-title-dark mb-0 inline-flex items-center overflow-hidden py-[16px] text-[18px] font-semibold text-ellipsis whitespace-nowrap">
      ...
      <!-- Recherche -->
      <div class="text-dark dark:text-title-dark border-regular dark:border-box-dark-up flex flex-wrap items-center justify-between border-b px-[15px] text-[17px] font-medium max-sm:h-auto max-sm:flex-col">
          ...
          <table class="min-w-full border border-gray-200 rounded-xl shadow-sm text-sm">
            ...
            <tbody class="text-gray-700">
              ...
              {:else if paginatedData.length === 0}
                <tr>
                  <td colspan="4" class="py-6 text-[12px] text-center text-gray-500">
...
<!-- Modales -->
<!-- <Modale bind:open={openAdd} size="xl" title="Créer un administrateur">
  <Add 
    bind:open={openAdd} 
    data={current_data} 
...
<Modale bind:open={openEdit} size="xl" title="Modifier un administrateur">
  <Edit 
    bind:open={openEdit} 
    data={current_data} 
    userUpdateId={user?.id}
...
<Modale bind:open={openShow} size="xl" title="Détails de l'administrateur">
  <Show 
    bind:open={openShow} 
    data={current_data} 
    on:updated={fetchData} 
...
<Modale bind:open={openDelete} size="xl" title="Supprimer l'administrateur">
  <Delete 
