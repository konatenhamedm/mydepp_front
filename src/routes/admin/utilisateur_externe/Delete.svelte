<script lang="ts">
	...
	async function confirmDelete() {
		...
		try {
			...
			const res = await apiFetch(true,`/user/delete/user/externe/` + data?.id,"DELETE");
...
<div class="space-y-4 rounded-lg bg-white p-1">
	...
	<div class="text-center">
		...
		<p class="text-gray-500">
			Êtes-vous sûr de vouloir supprimer cet enregistrement ({data?.email}) ? Cette action est
			irréversible.
		</p>
	</div>
