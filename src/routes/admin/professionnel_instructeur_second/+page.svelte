<script lang="ts">
  ...
  type ActionType = 'view' | 'edit' | 'delete' | 'add' | 'imputation';
  type Action = {
    action: ActionType;
    title: string;
    icon: string;
  ...
  const path: string = '/admin/dossiers-professionnels';
  ...
  function getTableHeaders(): string[] {
    const baseHeaders = ['Nom', 'Prénoms', 'Téléphone', 'Email', 'Professionnel de santé'];
    
    if (activeTab === 'valide') {
      return [...baseHeaders, 'Code', 'Imputation', 'Action'];
    } else {
      return [...baseHeaders, 'Imputation', 'Action'];
    }
  }

  // Fonction pour filtrer les actions disponibles selon le type d'utilisateur
  function getAvailableActions(userType: string): ActionType[] {
    // Seule l'action 'view' est disponible pour tous
    const availableActions: ActionType[] = ['view'];
    
    // Ajouter d'autres actions selon le type d'utilisateur
    if (userType === 'ADMINISTRATEUR' || userType === 'SOUS-DIRECTEUR-ETAB' || userType === 'SOUS-DIRECTEUR-PROF' || userType === 'DIRECTEUR') {
      availableActions.push('edit', 'delete', 'imputation');
    }
  ...
  onMount(async () => {
    ...
    const availableActions = getAvailableActions(user.type);
    actions = allActions.filter(action => availableActions.includes(action.action));
  });
  ...
  $: {
    if (permission) {
      // Re-filtrer les actions à chaque changement
      const availableActions = getAvailableActions(user.type);
      actions = allActions.filter(action => availableActions.includes(action.action));
    }
  }
