<script lang="ts">
	...
	async function SaveFunction() {
		isLoad = true;
		try {
			const res = await apiFetch(true,'/features/create', "POST", {
	...
	<div class="space-y-6">
		<form action="#" use:init>
			<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
				<!-- Champ pour le code du feature -->
				<InputSimple
					fieldName="code"
					label="Code"
					bind:field={feature.code}
					placeholder="Entrez le code du feature"
				/>

				<InputSimple
					fieldName="libelle"
					label="Libellé"
					bind:field={feature.libelle}
					placeholder="Entrez le nom du feature"
				/>
