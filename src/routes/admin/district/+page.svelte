<script lang="ts">
  ...
  const path: string = '/admin/district';
  ...
  async function fetchData() {
    loading = true;
    try {
      const res = await apiFetch(true, `/district/`);

      console.log(res);

      if (res) {
        main_data = res.data as District[];
    ...
    >
      ...
      <div class="p-[20px]">
        <div class="scrollbar overflow-x-auto">
          ...
          >
            <!-- Header -->
            <HeaderTable item={['Libelle', 'Region']} />
            <!-- Body -->
