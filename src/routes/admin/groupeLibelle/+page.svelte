<script lang="ts">
  ...
  import type {GroupeLibelle} from '../../../types';
  ...
  const path: string = '/admin/groupeLibelle';
  ...
  async function fetchData() {
    loading = true;
    try {
      const res = await apiFetch(true, `/libelleGroupe/`);

      console.log(res);

      if (res) {
        main_data = res.data as GroupeLibelle[];
...
>
  <Abercrome titre="Parametres" parent="Dashbord" current="groupe libelle" />
  <!-- Responsive Toggler -->
...
<Modale bind:open={openAdd} size="xl" title="Créer libelle groupe">
  <Add bind:open={openAdd} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openEdit} size="xl" title="Modifier libelle groupe">
  <Edit bind:open={openEdit} data={current_data} on:updated={fetchData} />
</Modale>
<Modale bind:open={openShow} size="xl" title="Détails d'libelle groupe">
