<script>
 ...
 async function handleSubmitNewTopic() {
  ...
  async function fetchForumData() {
    await axios.get(BASE_URL_API + "/forum").then((response) => {
      forumData = response.data.data;
    });
  }
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {#if showNewTopicModal==false && showTopicModal==false}
        ...
        <div class="divide-y divide-gray-100">
          ...
          {:else}
            {#each forumData as topic}
              <div class="p-6 hover:bg-gray-50 transition-colors" on:click={() => {topicSelected = topic; showTopicModal = true;}}>
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between">
                      <div class="flex-1">
                        ...
                        >
                          <span>Par <strong>{topic.user.personne.nom + ' ' + topic.user.personne.prenoms}</strong></span
                          ><span>•</span><span>{topic.user.typeUser}</span><span
                            >•</span
                      ...
                      <div class="text-right text-sm text-gray-500 ml-4">
                        <div class="flex items-center space-x-4 mb-2">
                          <div class="flex items-center space-x-1">
                            <i class="ri-message-3-line"></i><span>{topic.avis.length}</span>
                          </div>
                          <!-- <div class="flex items-center space-x-1">
      ...
      {:else if showTopicModal}
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          {:else}
            <ul class="space-y-4">
              {#each topicSelected.avis as avis}
      ...
      {:else if showNewTopicModal}
        <div class="bg-white rounded-xl shadow-lg p-6">
          <button class="mb-4 text-blue-600 hover:underline" on:click={() => {showNewTopicModal = false;}}>← Retour au forum</button>
          <h2 class="text-2xl font-bold mb-4">Nouvelle discussion</h2>
          <form on:submit|preventDefault={handleSubmitNewTopic}>
            <div class="mb-4">
              <label class="block text-gray-700 font-semibold mb-2" for="titre"
                >Titre</label
              >
              <input
                type="text"
                id="titre"
                bind:value={newTopic.titre}
